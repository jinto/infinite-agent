...
| 스킬 | 설명 |
|------|------|
| `autopilot` | 전체 파이프라인을 한번에 — think → plan → build → review → commit |
| `think` | 아이디어를 스펙으로 (기술 / 사업 / 개선) |
| `plan` | 합의 기반 플래닝 + TDD 태스크 분해 |
| `build` | 태스크 실행 — 혼자, 서브에이전트, 팀 중 알아서 선택 |
| `review` | 코드 리뷰 + 문제 있으면 직접 수정 |
...
태스크가 1개면 직접, 2-3개면 서브에이전트가 병렬로, 4개 이상이면 팀으로 나눠서 돌립니다.
...
Codex로 외부 리뷰를 받고, 고칠 수 있는 건 바로 고칩니다.
